...
okay, i added a couple of helper functions, one reads the parent node so it knows what node is the root of what, the other one just checks if a node is a leaf node and the next one create a Btree node, it checks if the tree is empty or if is not empty and assigns the block and parent IDs accordingly.

okay i have implemented the insert function, same idea as before and now when i test it, everything looks correct, i might reduce the minimal degree size so i can compare it with an online data structure visualizer to 100% make sure im right, if i test it out and it works correctly ill move on to the next command. It was easier to implement this time around because i already had the main idea from the c++ file so it was just a matter of converting to python syntax, i believe that the main issue i was having was with the insertnonfull() function, when i pass the parent node by reference, its not saved back onto the disk so the block IDs get wonky but its harder for me to see that with C++ code so making the switch to python was better. The next step now is to implement the search and print functions.

Okay so i have been able to implement the search function, how it works is that it firstly gets the root ID from the header and searches from the root, then reads the node at that block ID to see if the key exists there, if not then its checks which child pointer to follow, but im testing it right now and it says there was an error during search, i will read through what i have and fix any errors. Okay i realized what was wrong, when i called the readnode() function i didnt pass the file so the program kept returning None instead of the key, i will fix this and test now. Hmm....its still saying not found, let me try a simple number like 1 or something. Okay i see the problem, i didnt add keys to test.idx, i will run the insertTest.sh now. Okay, i get an error saying "split_node" is not defined, i realized i only placed a placeholder function call in place so i will implement the split node function now. when i tested it earlier i didnt need to split a node thats why i presumed it was fully working. Okay, i have implemented the split_node function, the key here is to make sure whenever i update the information of a node i need to write it back to disk, i had trouble doing that with C++ but python makes it a bit easier so lets see if i get any errors. Okay, no errors! now lets check the hexdump file and see if it works, perfect! it works! after inserting keys 1-30 in a b-tree that has a min degree of 20 (19 max nodes) the next available block ID should be 5 because the first 4 get used up after splits and what not and 5 is what the header shows as the next available block ID, also when i search for any key 1-30 i am able to do that, so the search functionality works! now onto the print function. The print function is probably my favourite one to implement because we can use BFS on the tree since its basially a graph, im currently learning about BFS in my algo class so it should be easy to implement with no errors. Okay i have implemented BFS but it prints the keys in level order which means right now 10,11 and 20, 21 are printed first, the project doesnt specify which way to print the keys but id prefer it if it was in increasing order so instead of using BFS i will use DFS, DFS shouldn't be too bad to implement as well. Okay, i have implemented DFS and now it prints it in increasing order so i am content! I will be taking a break for now and endng the session, but it was a very productive session.
12/06/2025 4:53 PM
This is the end of the session, i realized that C++ is its own beast and i should learn it when i dont have deadlines to meet lol but i was able to implement the insert function so it works now, i realized i was not saving a lot of information back to disk but my C++ code looked convoluted and hard to understand so i decided to scrap it and use python instead. I also added a lot of helper functions that help keep my code conscice and also implemented the print and search functions. In the next session i will implement the load and extract functions. the extract shouldnt be too bad, since i can call print and redirect the output to a csv file or something like that, not sure if i can do that in python but i will look into it.

12/07/2025 9:43 PM
Okay, this is the final stretch. In this session, i will finish the project by implementing the load and extract functions. I dont expect this to be too hard, since basically for the extract, i am looping through each node and printing its contents to the file, same for load. Another thing i need to add is the 3 node only constraint, but i will add that when im done with the load and extract functions.

Okay, i started with the extract function since it seems the easiest of the two, what i just did was use the dfs function from the print to traverse the B tree and add it into a csv with a new line character at the end so it always goes to the next line. Hmm thats strange, im getting a **sequence item 0: expected str instance, tuple found** error. not sure why that is, so its expecting a string instance but instead it found a tuple. So i need for format the tuple to a string first before writing it to the file. I wil try that now and test it again. Okay i just tested it out now and it works, thats great! now onto the load. For the load, i basically just read each line of the csv file then call the insert function to read it into the index file, should be easy peasy. Okay, i have implemented the load function, let me test it out and see if it works. Basically i just parse each line in the csv file, get the key and value then call the insert function and everything else is handled by the computer. Okay, i just tested it out and it works! im glad i saved these functions for last because they were the easiest to implement (asides from create) now i will do a comprehensive test on the program to make sure it meets specifications and will also add comments to my code. I just tested out the print function and it didnt work, oh i see its because i didnt include the third argument which is the key-value pairs, i will add that now. Okay it works. Great! I almost forgot i need to work on the 3 node in memory constraint. This should be easy and fun to implement because there have been a bunch of algorithims that deal with which page to evict in caches so ill look through the textbook and pick the easy one. Okay i found an easy one, its the least recently used algorithim, basically if the cache is full then it checks which current node is not being used as much and it replaces it with a new node, wasnt bad to implement. The most recently used nodes are kept in the back and least used are kept in front for easy deletion. i also added a clear cache function that clears the cache when each operation is done so as to free up memory. After a comprehensive test, i am done with the project. It was a very interesting project and i didnt know B trees were used in database systems> i basically created a database with i pretty cool!.

12/07/2025 11:43 PM
After 7 days ! I have been able to complete the project in its entirety. I will definately attempt this again using C++ after the break just to complete what i started but in this session i added the load and extract functions, i also polished the code by adding comments and i added a cache system to handle the 3 node constraint. The only thing left to do is create a readme, commit, and submit then im done!
